...
本项目采用`Scala`语言编写，旨在演示`SpringBoot`与`Dubbo`集成方式。

### 模块划分

* `dubbo-example-common` 是一个`maven`子模块，其中包含实体类定义和接口声明。
* `dubbo-example-provider` 也是一个`maven`子模块，包含接口的实现。
* `dubbo-example-consumer` 是另一个`maven`子模块，包含接口的引用，并包含了一个简单演示用的`Controller`。
